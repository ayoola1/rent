                 ...
                 <div class="panel panel-default sidebar-menu">
                     ...
                     {{--  <div class="box"> --}}
                        ...
                        <div class="row">
                           ...
                           <div class="col-md-6">
                               <p style="font-size: 14px;">You can adjust your payment plans to see other options</p>
                                 @include('includes.success_message')
                                <form action="{{route('calbreakdown',$user->id)}}" method="post">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                 
                                  <div class="form-group">
                                     <label for="">Amount of Rent (#):</label>
                                     <input type="text" class="form-control" value="{{$property->price}}" name="rent_amount" readonly="">
                                  </div>
                                  ...
                                  <div class="form-group">
                                     ...
                                     <select name="payment_plan" id="" class="form-control fin" required="">

                                       <?php

                                           if($user->payment_plan == '1 Month'){
                                              
                                              ?> <option selected="" value="{{$user->payment_plan}}">{{$user->payment_plan}}</option> <?php
                                           }
                                           else{
                                              
                                               ?> <option value="1 Month">1 Month</option> <?php
                                           }

                                            if($user->payment_plan == '3 Months'){
                                              
                                              ?> <option selected="" value="{{$user->payment_plan}}">{{$user->payment_plan}}</option> <?php
                                           }
                                           else{
                                              
                                               ?> <option value="3 Months">3 Months</option> <?php
                                           }

                                            if($user->payment_plan == '6 Months'){
                                              
                                              ?> <option selected="" value="{{$user->payment_plan}}">{{$user->payment_plan}}</option> <?php
                                           }
                                           else{
                                              
                                               ?> <option value="6 Months">6 Months</option> <?php
                                           }

                                            if($user->payment_plan == '12 Months'){
                                              
                                              ?> <option selected="" value="{{$user->payment_plan}}">{{$user->payment_plan}}</option> <?php
                                           }
                                           else{
                                              
                                               ?> <option value="12 Months">12 Months</option> <?php
                                           }
                                       ?>                                      
                           ...
                           <div class="col-md-6">
                               <div class="box">
                                 <p style="text-align: center;"><b>Your Payment Breakdown</b></p>
                                 <hr>
                                 <p style="text-align: center;">Amount:<b>#{{number_format($user->rent_amount,0,'',',')}} </b> </p>
                                 <hr>
                                 <p  style="text-align: center;">Monthly Pay:#<b>{{$user->monthly_plan ?number_format($user->monthly_plan,0,'',',') : '0'}}</b></p>
                                 <p  style="text-align: center;">Duration:{{$user->payment_plan ? $user->payment_plan : '0'}}</p>
                               </div> 
                           </div>
                        </div>
